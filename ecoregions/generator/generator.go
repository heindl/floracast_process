...
func main() {
	...
	fmt.Println("Feature Collection Area", fc_id_condensed.Area())
	...
	for _, feature := range fc_id_condensed.Features() {
		...
		cr.MultiPolygon = feature.MultipolygonArray()
		fmt.Println("Writing file", cr.EcoID)
		func() {
			fname := fmt.Sprintf("../generated_cache/EcoID_%d.go", cr.EcoID)
